...
bool world_representation::can_move_to(tetromino tetromino, short diff_x, short diff_y) {
    for (vector<short> pos: tetromino.get_positions()) {
        ...
        {
            cout<<"bound reached. can move in X axis\n";
            return false;
        }

        if (next_y_value < 0 || next_y_value >= height)
        {
            cout<<"bound reached. can move in Y axis\n";
            return false;
        }

        if (grid[next_x_value][next_y_value].filled)
        {
            cout<<"its already filled. cant move further\n";
            return false;
        }
    }
